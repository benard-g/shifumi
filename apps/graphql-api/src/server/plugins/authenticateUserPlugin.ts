...
import { REQUEST_HEADER_AUTHORIZATION } from '../constants';
...
export function createAuthenticateUserPlugin() {
  return FastifyPlugin(async (fastify) => {
    ...
    fastify.addHook('preHandler', async (req, reply) => {
      ...
      const bearerToken = req.headers[REQUEST_HEADER_AUTHORIZATION];
      ...
      try {
        ({ decodedAccessToken } = await authService.validateAuthTokens({
          accessToken,
        }));
      } catch (_err) {
        reply.code(401);
